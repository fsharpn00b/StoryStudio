...
let get_saved_game_display_data_from_storage (database_configuration : Database_Configuration)
...
*)
    Promise.create (fun resolve _ ->
        ...
        request_1?onsuccess <- (fun _ ->
            ...
            let tx = db?transaction (database_configuration.store_name, "readonly")
...
*)
    promise {
        try
            let! result = get_saved_game_from_storage_2 database_configuration existing_saved_game_id
            match result with
