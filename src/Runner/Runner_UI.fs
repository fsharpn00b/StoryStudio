...
*)
    let continuation () : unit =
        if not <| runner_state.runner_component_interfaces.current.menu.current.is_visible () &&
            not <| runner_state.runner_component_interfaces.current.image_map.current.is_visible () &&
            not <| runner_state.runner_component_interfaces.current.save_load.current.is_visible () &&
            not <| runner_state.runner_component_interfaces.current.configuration.current.is_visible () then
(* We must determine what the next command is before we can run it. *)
            do Runner_Queue.run runner_state reason
...
// TODO2 #plugins Consider expanding this check to all runner components. So far we have only had to check plugins because they are loaded asynchronously.
    let are_plugins_ready () : bool =
        runner_state.runner_component_interfaces.current.plugins |> Seq.exists (fun kv -> isNull kv.Value.interface_ref.current) |> not
...
let show_saved_game_screen
    ...
    do
        if runner_state.runner_component_interfaces.current.save_load.current.is_visible () then
            runner_state.runner_component_interfaces.current.save_load.current.switch action
        else
(* The save/load and configuration screens cannot be open at the same time. The player cannot switch from the configuration screen to the save/load game screen using the keyboard, but they can do so using the command menu. *)
            if runner_state.runner_component_interfaces.current.configuration.current.is_visible () then
                runner_state.runner_component_interfaces.current.configuration.current.hide ()
            show_saved_game_screen runner_state action
...
*)
    if not <| runner_state.runner_component_interfaces.current.save_load.current.is_visible () &&
        not <| runner_state.runner_component_interfaces.current.configuration.current.is_visible () then
        do undo_redo runner_state history true
...
(* When the save/load or configuration screen is visible, we do not want to undo or redo. *)
    if not <| runner_state.runner_component_interfaces.current.save_load.current.is_visible () &&
        not <| runner_state.runner_component_interfaces.current.configuration.current.is_visible () then
        do undo_redo runner_state history false
