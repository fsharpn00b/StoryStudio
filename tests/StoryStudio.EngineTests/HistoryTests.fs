...
let ``truncate_history keeps newest entries`` () =
    let a = Unchecked.defaultof<Runner_Saveable_State>
    let b = Unchecked.defaultof<Runner_Saveable_State>
    let c = Unchecked.defaultof<Runner_Saveable_State>
    let history = [a; b; c]
    let truncated = truncate_history history 2
    Assert.Equal(2, truncated.Length)
    Assert.Equal((history |> List.skip 1).Length, truncated.Length)
